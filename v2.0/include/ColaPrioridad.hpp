...
public:
    ...
    // Complejidad O(n): recorre el arreglo interno (sin respetar el orden
    // de extraccion, no importa para una consulta de existencia) y evalua
    // pred sobre cada elemento. No modifica el heap.
    // Uso previsto: verificar si un estudiante/recurso tiene una Solicitud
    // pendiente antes de permitir su eliminacion (ver Modulo 1 y 3 de main.cpp).
    template <typename Predicado>
    bool existe(Predicado pred) const;

    // Complejidad O(n): aplica 'accion' a cada elemento en el orden interno
    // del heap (NO es el orden de extraccion). Uso previsto: exportar todas
    // las solicitudes a CSV; al recargarlas con insertar() el heap se
    // reconstruye correctamente sin importar el orden de insercion.
